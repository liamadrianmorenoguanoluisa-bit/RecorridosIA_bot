import os, io, hmac, struct, time, base64, hashlib, json, logging, threading, urllib.request
import httpx
...
REMEDIOS = {
    ...
    "ELEMENTS SIN ETIQUETAS.": "VERIFICAR Y COLOCAR ETIQUETA ACRILICA CON EL CODIGO DE RUTA.",
...
def generar_excel(datos):
    ...
    for nov in r["novedades"]:
        ...
        for label, key in [("FECHA/HORA", "fecha"), ("MOTIVO", "motivo"), ("REMEDIO", "remedio"), ("TAREA PENDIENTE", "tarea_pendiente"), ("COORDENADAS", "coordenadas")]:
            fila = ws1.max_row + 1
            ws1.cell(fila, 1, label).font = Font(bold=True)
...
async def recv_tarea(update, ctx):
    ...
    await update.message.reply_text("Foto ANTES del mantenimiento novedad #" + str(idx+1) + "\nSin foto escribe: SALTAR")
    return FOTO_ANTES
...
async def recv_foto_antes(update, ctx):
    ...
    await update.message.reply_text("Foto DESPUES del mantenimiento novedad #" + str(idx+1) + "\nSin foto escribe: SALTAR")
    return FOTO_DESPUES
